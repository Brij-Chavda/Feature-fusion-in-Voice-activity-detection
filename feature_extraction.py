...
Original file is located at
    https://colab.research.google.com/drive/12Hbam5smr9UcqNEvJNFa3q4q-4MBm3h8
"""
...
def rastaplp(x, fs = 16000, win_time = 0.025, hop_time = 0.010, dorasta = True, modelorder = 18):
...
Xval = []
Xtest = []
for j in range(430, 630):
  print(j)
  filename = "/content/drive/My Drive/newtrainwithnoisep7/s" + str(j + 1) + ".wav"
  (data,Fs) = librosa.core.load(filename, sr = 16000)
  load_data = np.asarray(data)
  rasta = rastaplp(load_data)
  shape = np.shape(rasta)
  Mfcc = mfcc(data, Fs, numcep=19)
  mfcc_tr = np.transpose(Mfcc[0:shape[1]])
  delta_m = delta(mfcc_tr[1:19], order = 1)
  #print(np.shape(delta_m))
  
  delta_feature = delta(rasta[1:19, :], order = 1)
  feature_array = np.concatenate((rasta[1:19], delta_feature, delta_m), axis = 0)
  temp_transform = np.transpose(feature_array)
  n = np.floor(len(temp_transform) / 100)
  total_n = int(np.floor(n * 100))

  temp_data = temp_transform[0:int(total_n), :]
  temp_data = np.transpose(temp_data)
  if j < 430:
    if j == 0:
        Xtr = temp_data
    else:
        Xtr = np.hstack((Xtr, temp_data))
  elif j < 530:
    if j == 430:
        Xval = temp_data
    else:
        Xval = np.hstack((Xval, temp_data))
  else:
    if j == 530:
        Xtest = temp_data
    else:
        Xtest = np.hstack((Xtest, temp_data))
  num_array.append(int(total_n))

import numpy as np
label_shape = np.zeros(len(Xtr))
Ytr = []
Yval = []
Ytest = []
for j in range(630):
  filename = "/content/drive/My Drive/labelfile/s" + str(j + 1) + ".txt"
  label = []
  with open(filename) as file:
      for i in file:
          ...
          label.append(int(i))
  if j == 0:
    label_shape[0:num_array[0]] = label[0:num_array[j]]
  else:
    n = 0
    for k in range(j):
      n = n + num_array[k]
    label_shape[n:n + num_array[j]] = label[0:num_array[j]]
  if j == 430:
    ytr_len = n
  if j == 530:
    yval_len = n
  if j == 629:
    ytest_len = n + num_array[j]

import pickle
with open('/content/drive/My Drive/noisyXtrwithoutnorm36all.pkl', 'wb') as f:
  pickle.dump(Xtr, f)
import pickle
with open('/content/drive/My Drive/clean36Xvalwithoutnorm.pkl', 'wb') as f:
  pickle.dump(Xval, f)
import pickle
with open('/content/drive/My Drive/clean36Xtestwithoutnorm.pkl', 'wb') as f:
  pickle.dump(Xtest, f)
with open('/content/drive/My Drive/+1-1withendinglabel_seg.pkl', 'wb') as f:
  pickle.dump(Ytr, f)
with open('/content/drive/My Drive/+1-1withendingvallabel_seg.pkl', 'wb') as f:
  pickle.dump(Yval, f)
with open('/content/drive/My Drive/+1-1withendingtestlabel_seg.pkl', 'wb') as f:
  pickle.dump(Ytest, f)